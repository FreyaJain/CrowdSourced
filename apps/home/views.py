...
def index(request):
    if 'user_id' in request.session:
        ...
        user = None  # NULL should be replaced with None in Python
    ...
    for project in highest_rated_projects:
        image = project.image_set.all().first()  # Get the first image related to the project
        if image:  # Check if there is an image
            images.append(image.images.url)  # Add the image URL to the list
        else:
            images.append(None)  # You can append a default image URL here if needed
